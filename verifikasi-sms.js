(() => {
  ...
  form.addEventListener('submit', event => {
    ...
    if (failedAttempts >= MAX_ATTEMPTS) {
      setMessage('Batas percobaan tercapai. Buat kode baru untuk melanjutkan.', 'error');
      return;
    }
    ...
    if (/^\d{6}$/.test(enteredCode)) {
      ...
      setMessage(
        remaining > 0
          ? `Kode tidak sesuai. Sisa percobaan: ${remaining}.`
          : 'Batas percobaan tercapai. Buat kode baru.',
        'error'
      );
      resetInputs();
      return;
